...
export default function ChatCliente({ route }) {
  const { nome, pedido } = route.params;
  ...
  useEffect(() => {
    // Use 10.0.2.2 when testing on Android emulator. Change to localhost if using web or other env.
    const WS_HOST = "10.0.2.2"; // <--- se testar em PC com expo web use "localhost"
    const socket = new WebSocket(`ws://${WS_HOST}:3000`);

    socket.onopen = () => {
      console.log("conectado (cliente)");
      socket.send(JSON.stringify({ type: "role", role: "client", user: nome }));
    };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Cliente — {nome}</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View
            style={[
              styles.msgBox,
              item.sender === "cliente" ? styles.me : styles.peer,
            ]}
          >
